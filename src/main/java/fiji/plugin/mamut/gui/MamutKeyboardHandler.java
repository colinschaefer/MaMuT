...
public class MamutKeyboardHandler {
	...
	static {
		...
		DEFAULT_KEYBINGS
				.setProperty("T", BigDataViewerActions.MANUAL_TRANSFORM);

		final String[] numkeys = new String[] { "1", "2", "3", "4", "5", "6",
				"7", "8", "9", "0" };
		for (int i = 0; i < numkeys.length; ++i) {
			DEFAULT_KEYBINGS.setProperty(numkeys[i], "set current source " + i);
			DEFAULT_KEYBINGS.setProperty("shift " + numkeys[i],
					"toggle source visibility " + i);
		}
		...
		DEFAULT_KEYBINGS.setProperty("V", "maxproj");
		DEFAULT_KEYBINGS.setProperty("B", "toggle zdim dialog");
	}
	...
	protected InputMap readPropertyFile() {
		...
		try {
			if (!file.exists()) {
				...
				file = new File(MaMuT.class.getResource(
						"../../../mamut.properties").getPath());
			}
			final InputStream stream = new FileInputStream(file);
			config.load(stream);
		} catch (final Exception e) {
			System.out
					.println("MaMuT: cannot find the config file. Using default key bindings.");
	...
	protected void installKeyboardActions(final JComponent graphComponent) {
		NavigationActions.installActionBindings(viewer.getKeybindings(),
				viewer.getViewerPanel(), KeyProperties.readPropertyFile());
	...
	protected ActionMap createActionMap() {
		...
		map.put("increase spot radius",
				MamutActions.getIncreaseRadiusAction(mamut, viewer));
		map.put("decrease spot radius",
				MamutActions.getDecreaseRadiusAction(mamut, viewer));
		map.put("increase spot radius a lot",
				MamutActions.getIncreaseRadiusALotAction(mamut, viewer));
		map.put("decrease spot radius a lot",
				MamutActions.getDecreaseRadiusALotAction(mamut, viewer));
		map.put("increase spot radius a bit",
				MamutActions.getIncreaseRadiusABitAction(mamut, viewer));
		map.put("decrease spot radius a bit",
				MamutActions.getDecreaseRadiusABitAction(mamut, viewer));

		map.put("semi-auto tracking",
				MamutActions.getSemiAutoTrackingAction(mamut));

		map.put("toggle linking mode",
				MamutActions.getToggleLinkingModeAction(mamut,
						viewer.getLogger()));
		map.put("toggle link",
				MamutActions.getToggleLinkAction(mamut, viewer.getLogger()));

		map.put("show help", MamutActions.getShowHelpAction(viewer));

		map.put("toggle brightness dialog",
				MamutActions.getToggleBrightnessDialogAction(mamut));

		map.put("step time forward",
				MamutActions.getStepWiseTimeBrowsingAction(mamut, viewer, true));
