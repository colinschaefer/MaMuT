...
public class MamutViewerPanel extends ViewerPanel {
	...
	public MamutViewerPanel(final List<SourceAndConverter<?>> sources,
	...
	public void centerViewOn(final Spot spot) {
		...
		final double[] spotCoords = new double[] {
		...
		// JFrame.
		final double dx = display.getWidth()
				/ 2
				- (t.get(0, 0) * spotCoords[0] + t.get(0, 1) * spotCoords[1] + t
						.get(0, 2) * spotCoords[2]);
		final double dy = display.getHeight()
				/ 2
				- (t.get(1, 0) * spotCoords[0] + t.get(1, 1) * spotCoords[1] + t
						.get(1, 2) * spotCoords[2]);
